...
class GeneratorAutoEncoder(nn.Module):
    def __init__(self, in_channels=3, out_channels=3):
        super(GeneratorAutoEncoder, self).__init__()
    ...
    def forward(self, x):
        ...
        # d8 = self.down8(d7)    #d8:[1,512,1,1]
        # ipdb.set_trace()
        u1 = self.up1(d6, d6)  # u1:[1,1024,2,2]

        u2 = self.up2(u1, u1)  # u2:[1,1024,4,4]
        u3 = self.up3(u2, u2)  # u3:[1,1024,8,8]
        u4 = self.up4(u3, u3)  # u4:[1,1024,16,16]
        u5 = self.up5(u4, u4)  # u5:[1,512,32,32]
        # u6 = self.up6(u5, d1)  #u6:[1,256,64,64]
        # u7 = self.up7(u6, d1)  #u7:[1,128,128,128]
